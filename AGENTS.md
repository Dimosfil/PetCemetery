...
Pet Cemetery is a responsive memorial social network for people who want to
preserve the story of a deceased pet, choose how a real, approximate, symbolic,
or hidden place is presented, and receive respectful community support. The
current product surface is the web application in this repository. A possible
Unity application is a separate backlog product and is outside this runtime,
API, database, and release scope.

## Project Goal

Deliver a Docker-deployable responsive MVP in which a user can register, create
and manage a pet memorial, control memorial and location privacy, optionally
schedule a simple virtual ceremony, discover public memorials on a map, and
leave or receive respectful tributes. Completion requires server-side ownership
checks, public-coordinate privacy guarantees, PostgreSQL persistence, responsive
states, documented operation, and passing tests/build checks.
...
- Source: `app/`, `components/`, `lib/`, `db/`
- Tests: `tests/`
- Tools: `tools/` for durable development and agent tooling only
- Outputs/evidence/build artifacts: `.next/`, `coverage/`, Docker images
