...
- When the project-local config-service integration toggle is enabled, for
  ...
  config-service there too. When project integration is disabled, use only
  documented project-local runtime config and do not query or publish to
  config-service during ordinary startup.
