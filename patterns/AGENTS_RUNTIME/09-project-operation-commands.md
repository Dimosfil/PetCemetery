...
- Treat `gi reboot`, `ги ребут`, `gi restart`, and `ги рестарт` as requests to
  ...
  start covers the project. For local web/API services, when config-service
  integration is enabled, resolve the service id, port, URL, and neighboring
  endpoints through config-service before running a start command; fixed ports
  in local runbooks or examples do not authorize a fallback bind. When
  integration is disabled, use only documented project-local runtime config
  ...
  config-service record is missing while integration is enabled, use only the documented
  ...
  launches the full app set, use it only with the resolved local runtime values
  for web/API apps. Otherwise enumerate every documented app or
